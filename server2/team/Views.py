...
def create_team(request):
    ...
    team = Team(name=name, description=description, status=status, slug=slug)
    team.save()
    member = Member(id_user=user.id, level_asses='Admin', id_team=team.id)
    member.save()
    return return_team(request, {'tarefa': 'Tarefa cadastrada com sucesso!'})
...
def delete_team(request):
    ...
    authorization = Member.objects.get(id_user=user.id, id_team=team.id)
...
def update_team(request):
    ...
    if authorization:
        if authorization.level_asses == 'Admin':
            ...
            team.save()
            return return_team(request, {'edit': 'Tarefa editada com sucesso!'})
    return return_team(request, None)
